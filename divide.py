...
micro_path = "/store/kli/workdir/dhic/SuperHic/MicroAndHic/microC/HFF.mcool::/resolutions/25000"
...
#############################change#############
store_dir = "./inpdata/hff-block-per-pc/"
read_type = "mean"
################################################
...
for chr in chrs_list:
    ...
    pcvalue = pca(corr)
    pcmat = np.matmul(pcvalue[:,:].T,pcvalue[:,:]) * 1e3

    epic_l = []
    for f in filelist:
        seq = np.load("data/epi/hff/"+f+"_"+read_type+".npz")['chr'+chr]
    ...
    epicxl,epicxr,microy = divideblock(pcmat,epic_mat,bsize=128)
...
pd.DataFrame(res_tg).to_csv("inpdata/tg.csv")
